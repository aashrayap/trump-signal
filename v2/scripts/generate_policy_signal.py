...
Three source families of Trump communications:
  (1) Truth Social posts        -> local output/posts_all.csv  (~9,287 rows)
...
# Paths
# --------------------------------------------------------------------------- #
ROOT = Path(__file__).resolve().parents[1]            # tmp/trump-truth-social-alpha/v2
SCRIPTS = Path(__file__).resolve().parent             # .../v2/scripts
TASK = ROOT.parent                                    # tmp/trump-truth-social-alpha
REPO = ROOT.parents[2]                                # ai-investment-system
POSTS_CSV = TASK / "output" / "posts_all.csv"
PRICES_CSV = TASK / "output" / "market_prices.csv"
OUT = ROOT / "output"
CACHE = ROOT / "cache"
OUT.mkdir(parents=True, exist_ok=True)
CACHE.mkdir(parents=True, exist_ok=True)

# Import the three spike modules. They live next to this generator (copied from
# scratch/) so imports are robust regardless of cwd; fall back to scratch/.
# Make SCRIPTS authoritative and scratch the fallback, regardless of Python's
# automatic insertion of the script dir at sys.path[0]. Re-seat both so the
# curated gazetteer + edited extractor in SCRIPTS win over the scratch copies.
_SCRATCH = ROOT / "scratch"
for _p in (str(_SCRATCH), str(SCRIPTS)):   # inserted in reverse priority -> SCRIPTS ends first
    if _p in sys.path:
        sys.path.remove(_p)
...
def write_brief(events, ent, thm, tl, mkt, discards, counts, as_of,
    ...
    path = OUT / "policy-signal-brief.md"
